...
export function robotQueryFn(ctx: Context) {
  const {
    id /** 机器人 ID */,
    name /** 机器人名字 */,
    address /** 机器人地址 */,
...
export function robotAddFn(ctx: Context) {
  const { name, address, state, failureNum, admin, email, lat, lng } = ctx.query;
...
export function robotDeleteFn(ctx: Context) {
  const { id } = ctx.query;
...
export function robotUpdateFn(ctx: Context) {
  const { id, address, state, failureNum, email } = ctx.query;
  const robotList = readRobotList();
